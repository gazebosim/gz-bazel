...
load("@gz//bazel/skylark:py.bzl", "py_binary", "py_library", "py_test")
...
def gz_py_unittest(
    ...
    """
    helper = "//common/test_utilities:gz_py_unittest_main.py"
    if kwargs.pop("srcs", None):
        fail("Changing srcs= is not allowed by gz_py_unittest." +
             " Use gz_py_test instead, if you need something weird.")
    ...
    gz_py_test(
        ...
        main = helper,
        allow_import_unittest = True,
        _gz_py_unittest_shard_count = kwargs.pop("shard_count", None),
...
def gz_py_test(
    ...
    if kwargs.get("shard_count") != None:
        fail("Only gz_py_unittest can use sharding")
    shard_count = kwargs.pop("_gz_py_unittest_shard_count", None)
