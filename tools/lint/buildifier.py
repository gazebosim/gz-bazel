...
from ignition.tools.lint.find_data import find_data
from ignition.tools.lint.util import find_all_sources
...
def main(workspace_name="ignition"):
    ...
    if "-mode=diff" not in argv and "--mode=diff" not in argv:
        if os.getcwd().endswith(".runfiles/ignition"):
