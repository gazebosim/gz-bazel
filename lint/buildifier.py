...
from gz.bazel.lint.find_data import find_data
from gz.bazel.lint.util import find_all_sources

# These match data=[] in our BUILD.bazel file.
_BUILDIFIER = "external/buildifier/buildifier"
_TABLES = "bazel/lint/buildifier-tables.json"
...
def main(workspace_name="gz"):
    ...
    if "-mode=diff" not in argv and "--mode=diff" not in argv:
        if os.getcwd().endswith(".runfiles/gz"):
            print("ERROR: do not use 'bazel run' for buildifier in fix mode")
            print("ERROR: use bazel-bin/bazel/lint/buildifier instead")
            return 1
