...
def _genqrc(ctx):
    ...
    qrc_content = "<RCC>\n  <qresource prefix=\\\"/mainresources\\\">"
    for f in ctx.files.files:
        qrc_content += "\n    <file>%s</file>" % f.basename
    qrc_content += "\n  </qresource>\n</RCC>"
...
def qt_resource(name, files, qrc_file, **kwargs):
...
def qt_cc_binary(name, srcs, hdrs, linkopts, normal_hdrs = [], qrc = "resources", qrc_files = [], deps = None, **kwargs):
    ...
    native.genrule(
        name = qrc + "_gen",
        srcs = [qrc_file] + qrc_files,
        outs = [outfile],
        cmd = "echo $(rootpath %s) && cp $(location %s) . && rcc --name %s --output $(OUTS) %s" % (qrc_file, qrc_file, rsrc_name, qrc_file),
    )
    '''
